-- 015 — Fix HK staff login RPC 404 (safe to re-run)
-- Run in Supabase SQL Editor if verify_staff_login returns 404.
-- Or: npm run db:fix-login

CREATE EXTENSION IF NOT EXISTS pgcrypto;

GRANT USAGE ON SCHEMA public TO anon, authenticated, service_role;

-- Drop every overload of the staff-login RPCs (old p_ params, new names, etc.)
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Fallback table access if /rpc cache is stale
